document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('avatarPreview').src =
    user.avatar_url || '/images/default.jpg';

  // 🔥 AUTO ISI ADDRESS DARI API
  if (user.address) {
  ...
  productTotalDiv.innerText = `Total Produk: Rp${totalHargaProduk.toLocaleString('id-ID')}`;
  grandTotalDiv.innerText = `Total Bayar: Rp${(totalHargaProduk + shippingCost).toLocaleString('id-ID')}`;
    ...
    el.addEventListener('change', async () => {
      ...
      if (district_id && courier && weight > 0) {
        try {
          ...
          shippingCostDiv.innerText = `Ongkir (${layananOngkir}): Rp${shippingCost.toLocaleString('id-ID')}`;
          grandTotalDiv.innerText = `Total Bayar: Rp${(totalHargaProduk + shippingCost).toLocaleString('id-ID')}`;
    ...
    try {
      ...
      snap.pay(data.token, {
        ...
        onPending: async function(result) {
          ...
          resultBox.innerHTML = `<h3>⏳ Menunggu pembayaran</h3><pre>${JSON.stringify(result, null, 2)}</pre>`;
          window.location.href = `detailpembayaran.html?order_id=${result.order_id}`;
        },
        ...
        onClose: function() {
          resultBox.innerHTML = `
            ...
            <p>Anda menutup popup sebelum menyelesaikan pembayaran. Klik "Bayar Sekarang" lagi jika ingin melanjutkan.</p>
          `;
        }
      });
    } catch (err) {
